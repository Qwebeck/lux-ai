...
class ActionPreprocess(nn.Module):
    ...
    def forward(self, action_feature):
        ...
        direction_emb = self.direction_emb(
            action_feature['direction'][b, x, y].long())
        resource_emb = self.resource_emb(
            action_feature['resource'][b, x, y].long())
        repeat_emb = self.repeat_emb(
            (action_feature['repeat'][b, x, y] != 0).long())
...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.global_fc = nn.Sequential(
            nn.Linear(ModelParam.global_feature_dims,
                      ModelParam.global_emb_dim),
            nn.LeakyReLU(),
        )

        input_channel = ModelParam.global_emb_dim + \
            ModelParam.map_channel + ModelParam.action_emb_dim
        self.map_conv = nn.Sequential(
            nn.Conv2d(input_channel, ModelParam.all_channel,
                      kernel_size=1, stride=1, padding=0, bias=True),
            nn.LeakyReLU(),
            nn.Conv2d(ModelParam.all_channel, ModelParam.all_channel,
                      kernel_size=1, stride=1, padding=0, bias=True),
            nn.LeakyReLU(),
        )
        ...
        self.critic_head = nn.Sequential(
            nn.Conv2d(all_channel, all_channel, kernel_size=1,
                      stride=1, padding=0, bias=True),
            nn.LeakyReLU(),
            nn.Conv2d(all_channel, 1, kernel_size=1,
                      stride=1, padding=0, bias=True),
        )
    ...
    def _init_parameters(self):
        for name, param in self.named_parameters():
            if name[-len('weight'):] == 'weight':
                nn.init.orthogonal_(
                    param, gain=1 / np.power(np.prod(param.shape), 1 / 5))
