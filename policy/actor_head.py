...
def sample_from_categorical(logits, va, action=None):
    ...
    if n > 0:
        ...
        action = torch.zeros(torch.Size(
            (0, )), device=logits.device, dtype=torch.long)
        logp = torch.ones(torch.Size((0, )),
                          device=logits.device, dtype=torch.float32)
        entropy = torch.zeros(torch.Size(
            (0, )), device=logits.device, dtype=torch.float32)
...
class ActorHead(nn.Module):
    ...
    def __init__(self, channel_sz) -> None:
        ...
        if not EnvParam.rule_based_early_step:
            self.bid = nn.Sequential(
                nn.Conv2d(channel_sz, 1, kernel_size=1,
                          stride=1, padding=0, bias=True),
                nn.ReLU(),
                nn.Flatten(1),
                nn.Linear(EnvParam.map_size * EnvParam.map_size,
                          ActDims.bid, bias=True),
            )
        ...
        self.factory_act = nn.Linear(
            channel_sz, ActDims.factory_act, bias=True)

        self.unit_act_type = nn.Linear(
            channel_sz, ActDims.robot_act, bias=True)
        ...
        self.unit_act_transfer = nn.ModuleDict({
            ...
            "resource":
            nn.Sequential(
                nn.Linear(channel_sz, (ActDims.direction *
                          ActDims.resource), bias=True),
                nn.Unflatten(1, (ActDims.direction, ActDims.resource)),
            ),
            "amount":
            nn.Sequential(
                nn.Linear(channel_sz, (ActDims.direction *
                          ActDims.resource * self.amount_head_dim), bias=True),
                nn.Unflatten(
                    1, (ActDims.direction, ActDims.resource, self.amount_head_dim)),
            ),
            "repeat":
        ...
        self.unit_act_pickup = nn.ModuleDict({
            ...
            "amount":
            nn.Sequential(
                nn.Linear(channel_sz, (ActDims.resource *
                          self.amount_head_dim), bias=True),
    ...
    def bid_actor(self, x, va, action=None):
        logp, action, entropy = sample_from_categorical(
            self.bid(x), va, action)
        return logp, action, entropy
    ...
    def spawn_actor(self, x, va, action=None):
        ...
        if mask.any():
            ...
            if self.spawn_distribution == "categorical":
                def sample_amount_fn(logits, action): return sample_from_categorical(
                ...
                def sample_amount_fn(logits, action): return (
    ...
    def unit_actor(self, x, va, action=None):
        ...
        output_action = torch.zeros(
            (x.shape[0], len(UnitActChannel)), device=x.device)
    ...
    def transfer_actor(self, x, va, action=None):
        ...
        # logits
        params = {name: layer(x)
                  for name, layer in self.unit_act_transfer.items()}
    ...
    def forward(self, x, va, action=None):
        ...
        output_action['factory_act'] = _put_into_map(
            factory_action, factory_pos)
        ...
        if not EnvParam.rule_based_early_step:
            ...
            bid_logp, bid_action, bid_entropy = self.bid_actor(
                x, va['bid'], bid_action)
            logp += bid_logp
            entropy += bid_entropy
            ...
            spawn_logp, spawn_action, spawn_entropy = self.spawn_actor(
                x, va['factory_spawn'], spawn_action)
            logp += spawn_logp
