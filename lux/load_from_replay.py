...
import gzip
from lux.kit import obs_to_game_state, GameState, Board, json_obs_to_game_state
...
def get_obs_action_from_json(json_dict: dict, max_length=20):
    json_obs_0 = list(map(lambda i: json.loads(
        i[0]['observation']['obs']), json_dict['steps']))
    ...
    for i, o in enumerate(obs):
        if isinstance(o['player_0']['board']['rubble'], dict):
            if o['player_0']['board']['rubble'] == {}:
                o['player_0']['board']['rubble'] = deepcopy(
        ...
        if isinstance(o['player_1']['board']['rubble'], dict):
            o['player_1']['board']['rubble'] = deepcopy(
                o['player_0']['board']['rubble'])

        if 'ore' not in o['player_0']['board'].keys():
            o['player_0']['board']['ore'] = deepcopy(
                obs[i-1]['player_0']['board']['ore'])
        if 'ore' not in o['player_1']['board'].keys():
            o['player_1']['board']['ore'] = deepcopy(
                o['player_0']['board']['ore'])

        if 'ice' not in o['player_0']['board'].keys():
            o['player_0']['board']['ice'] = deepcopy(
                obs[i-1]['player_0']['board']['ice'])
        if 'ice' not in o['player_1']['board'].keys():
            o['player_1']['board']['ice'] = deepcopy(
                o['player_0']['board']['ice'])

        if isinstance(o['player_0']['board']['lichen'], dict):
            if o['player_0']['board']['lichen'] == {}:
                o['player_0']['board']['lichen'] = deepcopy(
        ...
        if isinstance(o['player_1']['board']['lichen'], dict):
            o['player_1']['board']['lichen'] = deepcopy(
                o['player_0']['board']['lichen'])

        if isinstance(o['player_0']['board']['lichen_strains'], dict):
            if o['player_0']['board']['lichen_strains'] == {}:
                o['player_0']['board']['lichen_strains'] = deepcopy(
        ...
        if isinstance(o['player_1']['board']['lichen_strains'], dict):
            o['player_1']['board']['lichen_strains'] = deepcopy(
                o['player_0']['board']['lichen_strains'])
    return obs
...
def replay_to_state_action(path):
    ...
    json_obs, json_action_0, json_action_1 = get_obs_action_from_json(
        data, max_length=max_length)
    first_step = 0 - json_obs[0]['real_env_steps'] + 1

    actions = dict(player_0=list(), player_1=list())
    obs = list(map(lambda i: {agent: deepcopy(json_obs[i]) for agent in [
               'player_0', 'player_1']}, range(len(json_obs))))
    obs = check(obs)
...
if __name__ == "__main__":
    obs_, actions, valid_actions = replay_to_state_action(
        path="../test.json", max_length=1000)
    ...
    agent = Net()
    def np2torch(x, dtype): return torch.tensor(x).type(dtype)
    agent.bc_new(
        ...
        tree.map_structure(lambda x: np2torch(
            x, torch.int16), mb_obs['action_feature']),
