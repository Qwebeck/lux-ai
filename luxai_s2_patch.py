...
from luxai_s2.env import (ActionsByType, LuxAI_S2, Position,
                          TransferAction, move_deltas, resource_to_name)
...
from luxai_s2.spaces.obs_space import FactoryIDSpace, FactionString, UnitIDSpace, UnitTypeSpace
...
def reset(self, seed=None):
    ...
    board = Board(seed=self.seed_rng.randint(
        0, 2**32 - 1, dtype=np.int64), env_cfg=self.env_cfg)
...
def _handle_movement_actions(self: LuxAI_S2, actions_by_type: ActionsByType):
    ...
    for unit, move_action in actions_by_type["move"]:
        ...
        if move_action.move_dir != 0:
            ...
            target_pos = (unit.pos + move_action.dist *
                          move_deltas[move_action.move_dir])
    ...
    for pos_hash, units in new_units_map.items():
        ...
        if len(heavy_entered_pos[pos_hash]) > 1:
            ...
            (most_power_unit, next_most_power_unit) = get_top_two_power_units(
                units, UnitType.HEAVY)
...
def install_patch():
    ...
    FactoryIDSpace._shape = (1,)
    FactoryIDSpace.dtype = str
    FactionString._shape = (1,)
    FactionString.dtype = str
    UnitIDSpace._shape = (1,)
    UnitIDSpace.dtype = str
    UnitTypeSpace._shape = (1,)
    UnitTypeSpace.dtype = str
