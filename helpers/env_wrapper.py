...
class LuxEnv(gym.Env):
    ...
    def __init__(self, kaggle_replays=None, **kwargs):
        ...
        self.action_space = get_action_space(self.rule_based_early_step)
        self.observation_space = get_observation_space()
        self.single_vas_space = self.get_va_space()

    def reset_proxy(self):
        assert not ((EnvParam.init_from_replay_ratio != 0.)
                    and (EnvParam.map_size != EnvConfig.map_size))
        if self.kaggle_replays and EnvParam.init_from_replay_ratio != 0:
            ...
            while True:
                ...
                obs = self.proxy.reset(
                    seed=np.random.SeedSequence().generate_state(1))
                if EnvParam.map_size < self.proxy.env_cfg.map_size:
                    pos = np.random.randint(
                        0, self.proxy.env_cfg.map_size - EnvParam.map_size, size=2)
            ...
            self.proxy.load_from_replay = False
            self.proxy.reset(seed=np.random.SeedSequence().generate_state(1))
    ...
    def reset(self):
        obs = self.reset_proxy()
        self.agents = {player: Player(player, self.env_cfg)
        ...
        if self.rule_based_early_step:
            while self.proxy.state.real_env_steps < 0:
                ...
                for player in self.proxy.agents:
                    o = obs[player]
                    a = self.agents[player].early_setup(
                        self.proxy.env_steps, o)
                    actions[player] = a
        ...
        for player_id, player in enumerate(self.proxy.agents):
            o = obs[player]
            self.game_state[player_id] = obs_to_game_state(
                self.proxy.env_steps, self.env_cfg, o)
        obs_list, global_info = self.feature_parser.parse(
            obs, env_cfg=self.env_cfg)
        self.reward_parser.reset(
            self.game_state, global_info, self.proxy.state.stats)

        return obs_list

    def step(self, actions):
        actions, action_stats = self.action_parser.parse(
            self.game_state, actions)
        obs, rewards, dones, infos = self.proxy.step(
            actions)  # interact with env
        self.real_obs = obs
        for player_id, player in enumerate(self.proxy.agents):
            o = obs[player]
            self.game_state[player_id] = obs_to_game_state(
        ...
        env_stats_logs = self.feature_parser.log_env_stats(
            self.proxy.state.stats)
    ...
    def eval(self, own_policy, enemy_policy):
        def np2torch(x, dtype): return torch.tensor(x).type(dtype).cuda()
        ...
        while not done:
            ...
            for id, policy in zip([own_id, enemy_id], [own_policy, enemy_policy]):
                ...
                _, _, raw_action, _ = policy(
                    np2torch([obs_list['player_'+str(id)]
                             ['global_feature']], torch.float32),
                    np2torch([obs_list['player_'+str(id)]
                             ['map_feature']], torch.float32),
                    tree.map_structure(lambda x: np2torch(
                        [x], torch.int16), obs_list['player_'+str(id)]['action_feature']),
                    tree.map_structure(lambda x: np2torch(
                        [x], torch.bool), valid_action)
                )
    ...
    def get_va_space(self):
        space = {
            'factory_act': spaces.MultiBinary([4, 48, 48]),
            'move': spaces.MultiBinary([5, 2, 48, 48]),
            'transfer': spaces.MultiBinary([5, 5, 2, 48, 48]),
            'pickup': spaces.MultiBinary([5, 2, 48, 48]),
            'dig': spaces.MultiBinary([2, 48, 48]),
            'self_destruct': spaces.MultiBinary([2, 48, 48]),
            'recharge': spaces.MultiBinary([2, 48, 48]),
            'do_nothing': spaces.MultiBinary([48, 48])
        }
        if not self.rule_based_early_step:
            space.update({'bid': spaces.MultiBinary(
                11), 'factory_spawn': spaces.MultiBinary([48, 48])})
        space = spaces.Dict(
            space
        )
    ...
    def concatenate_action(self, action_list):
        bs = len(action_list)
        action_shape_each_player = get_action_space(self.rule_based_early_step)
...
class LuxSyncVectorEnv(gym.vector.AsyncVectorEnv):
    def __init__(self, env_fns, observation_space=None, action_space=None, copy=True, shared_memory=False, worker=lux_worker):
        super().__init__(env_fns=env_fns, observation_space=observation_space,
                         action_space=action_space, copy=copy, shared_memory=shared_memory, worker=worker)
        dummy_env = env_fns[0]()
        self.single_vas_space = dummy_env.get_va_space()
    ...
    def get_valid_actions(self, player_id):
        ...
        self.vas = concatenate(
            valid_actions, self.vas, self.single_vas_space
        )
    ...
    def _process_eval_resluts(self, results):
        if self.num_envs == 1:
            ...
            episode_length, return_own, return_enemy = results[:,
                                                               0], results[:, 1], results[:, 2]
            results = {
